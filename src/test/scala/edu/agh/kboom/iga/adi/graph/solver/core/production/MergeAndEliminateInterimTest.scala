...
sealed class MergeAndEliminateInterimTest extends SubjectSpec
  ...
  "emit" when {
    ...
    "applied on left child" should {
      ...
      "is not empty" in {
        MergeAndEliminateInterim.emit(srcElement, dstElement) shouldNot be(null)
      }

      "emits matrix A translated by 1,1" in {
        MergeAndEliminateInterim.emit(srcElement, dstElement).ca shouldBe fromVector(6, 6)(
      ...
      "emits matrix B translated by one row" in {
        MergeAndEliminateInterim.emit(srcElement, dstElement).cb shouldBe fromVector(6, 14)(
