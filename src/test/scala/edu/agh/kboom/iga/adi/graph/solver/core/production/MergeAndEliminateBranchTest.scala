...
class MergeAndEliminateBranchTest extends SubjectSpec
  ...
  "emit" when {
    ...
    "applied on left child" should {
      ...
      "is not empty" in {
        MergeAndEliminateBranch.emit(srcElement, dstElement) shouldNot be(null)
      }

      "emits matrix A translated by 1,1" in {
        MergeAndEliminateBranch.emit(srcElement, dstElement).ca shouldBe fromVector(6, 6)(
      ...
      "emits matrix B translated by one row" in {
        MergeAndEliminateBranch.emit(srcElement, dstElement).cb shouldBe fromVector(6, 14)(
