...
import edu.agh.kboom.iga.adi.graph.solver.core.{Mesh, SplineSurface, StaticProblem}
...
class IterativeSolverIT extends AbstractIT {
  ...
  private def runSolver(solver: IterativeSolver)(p: StaticProblem): SplineSurface = {
    var lastSurface: Option[SplineSurface] = None
    solver.solve(p, (surface, si) => {
      lastSurface = Some(surface)
      si match {
        case StepInformation(step, _) => if (step < 2) Some(p) else None
      }
    })
    lastSurface.get
  }
  ...
  "running solver for" when {
    ...
    "element count is 12x12" should {
      ...
      "should produce valid results for f(x,y) = 1" in new SolverContext(12) {
        val finalSurface = runSolver(iterativeSolver)(OneProblem)
      ...
      "should produce valid results for f(x,y) = x + y" in new SolverContext(12) {
        val finalSurface = runSolver(iterativeSolver)(LinearProblem)
