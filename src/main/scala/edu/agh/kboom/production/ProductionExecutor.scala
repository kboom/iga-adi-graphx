...
object ProductionExecutor {
  ...
  def run(p: Production[Any], e: Element)(implicit ctx: IgaTaskContext): Unit =
    doRun(p, BoundElement(Vertex.vertexOf(ctx.vid)(ctx.mc.xTree()), e))

  def runOnLeaf(p: Production[Any], e: BoundElement)(implicit ctx: IgaTaskContext): Unit = p match {
    case s: InitializeLeaf => InitializeLeaf.run(s, e)
  }

  private def doRun(p: Production[Any], e: BoundElement)(implicit ctx: IgaTaskContext): Unit = e.v match {
